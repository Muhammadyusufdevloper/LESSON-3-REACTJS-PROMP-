...
import "./Header.scss"
import { navbarLeftData } from '../../static'
import { navbarRightData } from '../../static'
import { CiSearch,CiShoppingCart } from "react-icons/ci";
import { FaRegUser } from "react-icons/fa";


const Header = () => {
    const navbarLeftItem = navbarLeftData.map((link)=>(
        <li className='header__leaft__item' key={link.id}>
            <a href="#" className='header__left__link'>
                {link.title}
            </a>
        </li>
    ))
    const navbarRightItem = navbarRightData.map((link)=>(
        <li className='header__right__item' key={link.id}>
    ...
    <>
        <header className='header'>
            <nav className='header__navbar container'>
                <div className='header__left__boxes'>
                    <a href="#" className='header__logo--link'>
                        <svg xmlns="http://www.w3.org/2000/svg" width="120" height="32" viewBox="0 0 120 32" data-di-res-id="dbfbbed4-19e7912" data-di-rand="1715002031604" role="presentation" aria-hidden="true" tabindex="-1">
                ...
                <div className='header__right__boxes'>
                    <ul className='header__right__list'>
                        {navbarRightItem}
                    </ul>
                    <div className='header__right__btns'>
                        <button className='header__right__btn'>
                            <CiSearch className='header__right__btn__icon'/>
                        </button>
                        <button className='header__right__btn'>
                            <CiShoppingCart className='header__right__btn__icon' />
                        </button>
                        <button className='header__right__btn'>
                            <FaRegUser className='header__right__btn__icon' />
                        </button>
                    </div>
                </div>
            </nav>
